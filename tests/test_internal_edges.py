...
def test_normalize_params_returns_plain_string_dict_for_mapping() -> None:
    # Arrange
...
def test_invalid_datetime_value_raises_structured_error() -> None:
    ...
    # Assert
    assert exc_info.value.to_list()[0].get("type") == "query.type_error.datetime"
...
def test_unknown_contract_type_falls_back_to_string_coercion() -> None:
    ...
    # Act
    mongo = query.parse({"custom": "raw"}).output
...
def test_make_scalar_coercer_falls_back_to_string_coercer() -> None:
    # Arrange
    coercer = make_scalar_coercer(CustomType)
...
def test_raw_mongo_emitter_applies_aliases_and_sort_direction() -> None:
    ...
    # Act
    mongo = MongoEmitter().emit(ast, compile_contract(fields))
...
def test_raw_mongo_emitter_merges_eq_after_range_filter() -> None:
    ...
    # Act
    mongo = MongoEmitter().emit(ast, compile_contract({}))
