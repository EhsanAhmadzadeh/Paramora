...
class MongoEmitter(QueryEmitter[MongoQuery]):
    ...
    def emit(self, ast: QueryAst, fields: Mapping[str, QueryField]) -> MongoQuery:
        ...
        for node in ast.filters:
            ...
            field_name = (
                field.backend_name(node.field) if field is not None else node.field
            )
            _merge_filter_node(filter_doc, field_name, node.op, node.value)
    ...
    def emit_compiled(self, ast: QueryAst, contract: CompiledContract) -> MongoQuery:
...
def _merge_filter_node(
    filter_doc: MongoFilter, field_name: str, operator: str, value: Any
) -> None:
    if operator == "eq":
